...
class MainActivity : AppCompatActivity() {
    ...
    private fun parseData() {
        ...
        recyclerView.adapter = AdapterClass(dataList,object : AdapterClass.onItemClickListner{
            override fun onItemClick(position: Int) {

                val intent = Intent(applicationContext , CompanyDetailActivity::class.java)
                intent.putExtra("image" , dataList[position].dataImg)
                intent.putExtra("name" , dataList[position].dataName)
                intent.putExtra("description" , dataList[position].dataDescription)
